            ...
            try
            {
                using (PruebaDataContext bd = new PruebaDataContext())
                {
                    ...
                    if (rpta ==1)
                    {
                        ...
                        Session["tipoUsuario"] = bd.Usuario.Where(p => p.NOMBREUSUARIO == usuario && p.CONTRA == contraCifrada).First().TIPOUSUARIO;
                        ...
                        {
                            Variable.acciones.Add(item.act);
                            Variable.controladores.Add(item.cts);
                            Variable.mensaje.Add(item.msj);
                        }
                    }
                }
            }
